...
void RAWGroupping (uint16_t Numsta, uint16_t NGroups, uint16_t NumSlot, uint16_t trial, string RAWConfigPath, uint32_t beaconinterval, uint32_t pageSliceCount, uint32_t pageSliceLen, uint16_t crossSlotBoundary, uint16_t mcsindex = 0)
{
    uint16_t NRawSta;
    uint16_t NRawGroups;
    uint16_t NstaPerRaw;
    ...
    if (pageSliceCount == 0)
    {
     	...
     	aid_end = Numsta;
     	SlotDurationCount = ((beaconinterval-5600-(NRawGroups-1)*(120*SlotDurationCount+500))/(SlotNum) - 500)/120;
 		newfile << RawControl << "\t" << SlotCrossBoundary << "\t" << SlotFormat << "\t" << SlotDurationCount << "\t" << SlotNum << "\t" << pageIndex << "\t" << aid_start << "\t" << aid_end << "\t" << "\n";
 		aid_start = aid_end + 1;
 		groupID++;
    }
...
int main (int argc, char *argv[])
{
  ...
  uint32_t pageSliceLen, pageSliceCount;
    ...
    cmd.AddValue ("MCSIndex", "MCS index, 2MHz channel is considered only", mcsindex);
    cmd.Parse (argc,argv);
    
    std::cout << "NRawSta=" << NRawSta <<  " NGroup=" << NGroup << " beaconinterval=" << beaconinterval << " RAWConfigPath=" << RAWConfigPath << " crossSlotBoundary=" << (int)crossSlotBoundary << " mcsindex=" << mcsindex << std::endl;
    
    void RAWGroupping (uint16_t, uint16_t, uint16_t, uint16_t, string, uint32_t, uint32_t, uint32_t, uint16_t, uint16_t);
    


 
    
  RAWGroupping (NRawSta, NGroup, NumSlot, trial, RAWConfigPath, beaconinterval, pageSliceCount, pageSliceLen, crossSlotBoundary, mcsindex);
