...
StaWifiMac::GoToSleepBinary (int value)
{
     if (IsAssociated() && !receivingBeacon)
     {
        ...
        else if ( value == 1 && !m_low->GetPhy()->IsStateSleep())
         {
...
*/
...

	   {
		   if (testtrackit)
			   NS_LOG_DEBUG(
					   "At " << Simulator::Now().GetSeconds() << " s AID " << this->GetAID(0) << " switches to SLEEP because it has packets in queue but !waitingack && !outsideraw && !stationrawslot (SleepIfQueueIsEmpty).");
		   m_low->GetPhy()->SetSleepMode();
	   }
	   ...
	   if (!outsideraw && !stationrawslot)
	   {
		   if (testtrackit)
			   NS_LOG_DEBUG(
					   "At " << Simulator::Now().GetSeconds() << " s AID " << this->GetAID(0) << " switches to SLEEP because !outsideraw && !stationrawslot (SleepIfQueueIsEmpty).");
		   m_low->GetPhy()->SetSleepMode();
	   }
...
StaWifiMac::BeaconWakeUp (void)
{
	...
	if (m_low->GetPhy()->IsStateSleep())
	{
		...
		if (testtrackit)
			NS_LOG_DEBUG(
					"At " << Simulator::Now().GetSeconds() << " s AID " << this->GetAID(0) << " WAKES UP in BeaconWakeUp.");
...
{    
    ...
    Time os = Simulator::Now() + m_currentslotDuration[m_slotCounter];
    //NS_LOG_DEBUG ( m_low->GetAddress () << " Inside backoff scheduled " << Simulator::Now() << m_currentslotDuration);
    //NS_LOG_UNCOND ("StaWifiMac::RawSlotStartBackoff aid=" << this->GetAID() << " at " << Simulator::Now() << " m_currentslotDuration=" << m_currentslotDuration[m_slotCounter] << ", m_slotCounter=" << m_slotCounter);
    m_insideBackoffEvent = Simulator::Schedule(m_currentslotDuration[m_slotCounter], &StaWifiMac::InsideBackoff, this);
...
{    
	...
	if (stationrawslot)
	{
		...
		{
			if (testtrackit)
				NS_LOG_DEBUG (m_low->GetAddress () << ",enqueue in stationrawslot was sleeping," << Simulator::Now().GetSeconds());
			WakeUp();
		}
		else
		{
			if (testtrackit)
				NS_LOG_DEBUG (m_low->GetAddress () << ",enqueue in stationrawslot still awake," << Simulator::Now().GetSeconds());
		}
	}
  ...
  //std::cout << "m_qosSupported=" << m_qosSupported << std::endl;
  if (m_qosSupported)
    {
...
StaWifiMac::S1gBeaconReceived (S1gBeaconHeader beacon)
{
    ...
    //if (this->IsAssociated())
    //std::cout << "+++++StaWifiMac::S1gBeaconReceived, aid=" << this->GetAID(0) <<" m_rawStart=" << m_rawStart << ",m_inRawGroup=" << m_inRawGroup << ",m_pagedStaRaw=" << m_pagedStaRaw << ",m_dataBuffered=" << m_dataBuffered << std::endl;
  if (!m_aids.size()) // send assoication request when Staion is not assoicated
    {
      m_dca->AccessAllowedIfRaw (true);
    }
  if (m_rawStart && m_inRawGroup && m_pagedStaRaw && m_dataBuffered ) // if m_pagedStaRaw is true, only m_dataBuffered can access channel
    {
  ...
  else if (m_rawStart && m_inRawGroup && !m_pagedStaRaw  )
    {
      ...
      m_edca.find (AC_BE)->second->AccessAllowedIfRaw (false);
      m_edca.find (AC_BK)->second->AccessAllowedIfRaw (false);
      for (auto const& nextSlotStart: m_statSlotStart)
      {
    	  Simulator::Schedule(nextSlotStart, &StaWifiMac::RawSlotStartBackoff, this);
      }
    }
  ...
  if (hdr->GetAddr3 () == GetAddress ())
    ...
    if (m_pagedInDtim)
    {
    	...
    	*/
    	...
    	//schedule wake up for the next beacon
    	Time timeUntilNextDtim = MicroSeconds(beacon.GetBeaconCompatibility().GetBeaconInterval() * beacon.GetpageSlice().GetPageSliceLen()) - MicroSeconds(Simulator::Now().GetMicroSeconds() % beacon.GetBeaconCompatibility().GetBeaconInterval() * beacon.GetpageSlice().GetPageSliceLen());
    	Simulator::Schedule(timeUntilNextDtim, &StaWifiMac::WakeUp, this);
    	m_pagedInDtim = false;
    }
